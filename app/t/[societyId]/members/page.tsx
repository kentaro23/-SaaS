    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <p className="mt-2 text-xs text-slate-500">
          Excelで編集後にCSV保存してアップロードしてください（姓/名/姓かな/名かな/郵便番号/都道府県/市区町村/番地は必須）。重複する会員番号は更新されます。
          ...
          <tbody className="divide-y divide-slate-100">
            {members.map((m) => (
              <tr key={m.id}>
                <Td>{m.memberNo}</Td>
                <Td>
                  <div className="font-medium">{(m.familyName && m.givenName) ? `${m.familyName} ${m.givenName}` : m.name}</div>
                  <div className="text-xs text-slate-500">
                    {(m.kanaFamily && m.kanaGiven) ? `${m.kanaFamily} ${m.kanaGiven}` : (m.kana || "-")}
                  </div>
                </Td>

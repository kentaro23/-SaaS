...
import { meetingStatusLabel, meetingTypeLabel } from "@/lib/labels";
...
export default async function MeetingsPage({ params }: { params: Promise<{ societyId: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <Table>
          <thead><tr><Th>タイトル</Th><Th>種別</Th><Th>開催日時</Th><Th>状態</Th><Th>件数</Th><Th>操作</Th></tr></thead>
          <tbody className="divide-y divide-slate-100">
            {meetings.map((m) => (
              <tr key={m.id}>
                <Td><div className="font-medium">{m.title}</div><div className="text-xs text-slate-500">{m.location || m.onlineUrl || "-"}</div></Td>
                <Td>{meetingTypeLabel(m.type)}</Td>
                <Td>{formatDateTime(m.scheduledAt)}</Td>
                <Td><StatusBadge tone={m.status === "DONE" ? "green" : m.status === "SCHEDULED" ? "blue" : "yellow"}>{meetingStatusLabel(m.status)}</StatusBadge></Td>
                <Td>
                  <div className="text-xs">出欠 {m._count.attendances}</div>
                  <div className="text-xs">資料 {m._count.documents}</div>
                  <div className="text-xs">タスク {m._count.tasks}</div>
                </Td>
